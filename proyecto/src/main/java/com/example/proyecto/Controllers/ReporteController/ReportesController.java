...
public class ReportesController {
    ...
    // public String reporteProyecoctoOne(@RequestParam(value = "id_proyecto")Long
    // id_proyecto, Model model){
    ...
    // evaluacionesList.sort(Comparator.comparing(evaluacion ->
    // evaluacion.getId_evaluacion()));
    ...
    // }
    // c1.sort(Comparator.comparing(criterio ->
    // criterio.getPreguntas().getId_pregunta()));
    // c2.sort(Comparator.comparing(criterio ->
    // criterio.getPreguntas().getId_pregunta()));
    // c3.sort(Comparator.comparing(criterio ->
    // criterio.getPreguntas().getId_pregunta()));
    ...
    public String reporteProyecoctoOne(@PathVariable(name = "id_proyecto") Long id_proyecto, Model model) {
        ...
        // Usar BigDecimal para redondear a dos decimales
        BigDecimal promedio = new BigDecimal(proyecto.getPromedio_final());
        BigDecimal promedioRedondeado = promedio.setScale(2, RoundingMode.HALF_UP);

        // Establecer el valor redondeado en el proyecto
        proyecto.setPromedio_final(promedioRedondeado.doubleValue());

        model.addAttribute("proyecto", proyecto);

        List<CategoriaCriterio> categorias = categoriaCriterioService
    ...
    // @GetMapping("/ReporteProyecoctoOne")
    // public ResponseEntity<byte[]> reporteProyecoctoOne(@RequestParam(value =
    // "id_proyecto") Long id_proyecto) {
    // Proyecto proyecto = proyectoService.findOne(id_proyecto);
    // List<CategoriaCriterio> categorias =
    // categoriaCriterioService.obtenerPonderacionesPorProyecto(id_proyecto);
    // List<Ponderacion> ponderaciones =
    // ponderacionService.obtenerPonderacionesPorProyecto(id_proyecto);
    // List<Evaluacion> evaluaciones =
    // evaluacionService.obtenerNotasFinales(id_proyecto);
    ...
    // String rutaImagen = Paths.get("").toAbsolutePath().toString() +
    // "/proyecto/src/main/resources/static/assets/images/menbretado_uap.png";
    ...
    // // String nom_completo = personal.getPersona().getNombre() + " " +
    ...
    public static void addTitle(Proyecto proyecto, Document document) throws DocumentException {
        // BaseFont baseFont =
        // BaseFont.createFont(Paths.get("").toAbsolutePath().toString()+
        // "/planificacion/src/main/resources/static/assets/memo/timesnewromanbold.ttf",BaseFont.IDENTITY_H,
        // BaseFont.EMBEDDED);
        ...
        // BaseFont baseFont2 =
        // BaseFont.createFont(Paths.get("").toAbsolutePath().toString()+
        // "/planificacion/src/main/resources/static/assets/memo/timesnewroman.ttf",BaseFont.IDENTITY_H,
        // BaseFont.EMBEDDED);
        ...
        for (Estudiante estudiante : proyecto.getEstudiante()) {
            nombres += estudiante.getPersona().getNombres() + " " + estudiante.getPersona().getPaterno() + " "
                    + estudiante.getPersona().getMaterno() + " \n";
        }
        table.addCell(createCell(nombres, normal));
        table.addCell(createCell("DOCENTE ASESOR:", negrita));
        String docente = proyecto.getDocente().getPersona().getNombres() + " "
                + proyecto.getDocente().getPersona().getPaterno() + " "
                + proyecto.getDocente().getPersona().getMaterno();
